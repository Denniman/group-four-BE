...
export const getReview = async (req, res) => {
  try {
    ...
    if (getOneReview !== null) {
      res.status(200).json({ getReviews }).status('success').end();
    } else {
      res
        .status(404)
...
export const postReviews = async (req, res) => {
  try {
    ...
    if (!dishId) {
      res
        .status(400)
        .send({ message: `No dishId present for this review` })
        .end();
      return;
    }
...
export const deleteReviews = async (req, res) => {
  try {
    ...
    if (deleteAll.n > 0) {
      res
        .status(200)
        .send({ message: `deleted reviews for ${dishId}` })
        .end();
    } else {
      res
        .status(404)
        .send({ message: `There are no reviews for this dish id` })
        .end();
    }
  } catch (err) {
    res.status(400).send({ message: `Invalid dish id` }).end();
